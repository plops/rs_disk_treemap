                ...
                for k in 0..row.len() {
                    {
                        ...
                        .max(0.0);
                        children[idx].target_rect =
                            Rect::new(rect.x, current_y, row_thickness, item_h);
                        current_y += item_h
                    }
                }
            }
                ...
                for k in 0..row.len() {
                    {
                        ...
                        .max(0.0);
                        children[idx].target_rect =
                            Rect::new(current_x, rect.y, item_w, row_thickness);
                        current_x += item_w
                    }
                }
            }
                                ...
                                } else {
                                    {
                                        ...
                                        if remaining_rect.w <= 0.0 || remaining_rect.h <= 0.0 {
                                            for j in i + 1..node.children.len() {
                                                node.children[j].target_rect = Rect::new(
                                                    remaining_rect.x,
                                                    remaining_rect.y,
                                                    0.0,
                                                    0.0,
                        ...
                        match watcher.watch(root_path, mode) {
                            ...
                            Err(e) => {
                                ...
                                );
                                {
                                    ...
                                    if err_str.contains("os error 28")
        ...
        {
            {
                ...
                node.current_rect.x =
                    node.current_rect.x + (node.target_rect.x - node.current_rect.x) * tt;
                node.current_rect.y =
                    node.current_rect.y + (node.target_rect.y - node.current_rect.y) * tt;
                node.current_rect.w =
                    node.current_rect.w + (node.target_rect.w - node.current_rect.w) * tt;
                node.current_rect.h =
                    node.current_rect.h + (node.target_rect.h - node.current_rect.h) * tt;
                still_anim = true
            }
        } else {
                        ...
                        loop {
                            {
                                                ...
                                                if let Ok(rel) = path.strip_prefix(&target_dir) {
                                                    {
                                                        ...
                                                        {
                                                            let parent_comps = components
                                                                .split_at(components.len() - 1)
                                                                .0;
                                                            let item_name = components
                                                                [components.len() - 1]
                                                                .to_string_lossy();
                                                                    ...
                                                                    if meta.is_dir() {
                                                                        {
                                                                            ...
                                                                            if dir_needs_scan {
                                                                                if let Some(
                                                                                    ...
                                                                                    {
                                                                                        parent.children.push(FileNode::new(item_name.into_owned(), true, 0));
                                                                                        parent.is_sorted=false;
                                                                                        layout_dirty=true
                                                                                    }
                                                                                }
                                                                                ...
                                                                                {
                                                                                    let deep_tx = scan_tx_keep.clone();
                                                                                    let deep_abort =
                                                                                        abort_scan
                                                                                            .clone();
                                                                                    let deep_target =
                                                                                        path.clone();
                                                                                    let deep_scanners = active_scanners.clone();
                                                                                    thread::spawn(
                                                                                        move || {
                                                                                            scan_directory_recursive(&deep_target, &deep_tx, &deep_abort);
                                ...
                                }
                                {
                                    ...
                                    if layout_dirty
                                        && (!scanning
                                            || 100 < last_layout_time.elapsed().as_millis())
                                    {
                                        ...
                                        layout_treemap_sequential(
                                            &mut root_node,
                                            &mut layout_workspace,
                                        );
                                        layout_dirty = false;
                                        is_animating = true;
                                    ...
                                    if scanning {
                                        ...
                                        if is_animating {
                                            is_animating =
                                                update_animations(&mut root_node, dt, now)
                                        }
                                    }
                                    {
                                            ...
                                            || is_mouse_button_down(MouseButton::Middle)
                                        {
                                            {
                                                let delta =
                                                    (mouse_screen - last_mouse) / camera_zoom;
                                                camera_pos += delta
                                            }
                                        }
                                            ...
                                            if 1.00e-2 < wheel.abs() {
                                                {
                                                    ...
                                                    camera_zoom =
                                                        camera_zoom.clamp(5.00e-2, 1.00e+2);
                                                    {
                                                                        ...
                                                                        let status_text =
                                                                            if scanning {
                                                                                ...
                                                                                if watcher_mgr
                                                                                    .limit_hit
                                                                                {
                                                                                    format!("WATCHING (INOTIFY LIMIT: {} DIRS)", watcher_mgr.watched_dirs.len())
                                                                                } else {
                                                                                    if watcher_alive
                                                                                    {
                                                                                        backend_label(&watcher_mgr)
                                                                                    } else {
                                                                        ...
                                                                        {
                                                                            {
                                                                                ...
                                                                                if 1024.0
                                                                                    < node
                                                                                        .growth_rate
                                                                                {
                                                                                    info.push_str(&format!("  [Active: {}/s]", format_bytes(node.growth_rate as u64)))
                                                                                }
                                                                                draw_text(
                                                                                    &info,
                                                                                    14.0,
                                                                        ...
                                                                        } else {
                                                                            {
                                                                                ...
                                                                                draw_text(target_dir.to_string_lossy().as_ref(), 4.20e+2, 36.0, 14.0, LIGHTGRAY);
                                                                            }
                                                                        }
                                                                        next_frame().await;
                                                                    }
